...
function admin(req,res){const s=session(req);if(!s?.admin){json(res,401,{error:'Admin authentication required'});return null}return s}
...
async function route(req,res){
 try{
  ...
  if(req.method==='GET'&&p==='/api/bootstrap')return json(res,200,{crops:db.crops,centres:db.centres,slots:db.slots||seed.slots,slotCapacity:db.slotCapacity||{},settings:{payeeName:db.settings.payeeName,upiConfigured:!!db.settings.upiId},today:today(),adminConfigured:!!db.admin});
  if(req.method==='POST'&&p==='/api/admin/setup'){if(db.admin)return json(res,409,{error:'Admin is already configured'});const b=await body(req);if(!b.email||!b.password||b.password.length<8)return json(res,400,{error:'Email and password (8+ characters) are required'});const salt=newSalt();db.admin={email:b.email.toLowerCase().trim(),salt,hash:hash(b.password,salt)};save(db);audit('ADMIN_SETUP','Initial administrator account created');return json(res,200,{ok:true})}
  if(req.method==='POST'&&p==='/api/admin/login'){const ip=req.socket.remoteAddress||'unknown',a=attempts.get(ip)||{n:0,until:0};if(a.until>Date.now())return json(res,429,{error:'Too many failed attempts. Try again later.'});const b=await body(req);if(!db.admin)return json(res,400,{error:'Admin setup required'});const ok=b.email?.toLowerCase().trim()===db.admin.email&&crypto.timingSafeEqual(Buffer.from(hash(b.password||'',db.admin.salt)),Buffer.from(db.admin.hash));if(!ok){a.n++;if(a.n>=5){a.until=Date.now()+5*60e3;a.n=0}attempts.set(ip,a);return json(res,401,{error:'Invalid administrator credentials'})}attempts.delete(ip);const sid=crypto.randomBytes(32).toString('hex');sessions.set(sid,{admin:true,expires:Date.now()+15*60e3});res.setHeader('Set-Cookie',`cs_session=${sid}; HttpOnly; SameSite=Strict; Path=/; Max-Age=900`);audit('ADMIN_LOGIN','Administrator logged in');return json(res,200,{ok:true})}
  ...
  if(req.method==='GET'&&p==='/api/admin/me'){return json(res,200,{authenticated:!!session(req),configured:!!db.admin})}
