...
const sessions=new Map(), attempts=new Map(), otps=new Map(), farmerSessions=new Map(), otpRequests=new Map();
...
async function route(req,res){
 try{
  ...
  if(req.method==='POST'&&p==='/api/otp'){const b=await body(req),phone=String(b.phone||''),ip=req.socket.remoteAddress||'unknown',key=ip+':'+phone,r=otpRequests.get(key)||{count:0,reset:Date.now()+10*60e3};if(!/^\d{10}$/.test(phone))return json(res,400,{error:'Enter a valid 10-digit mobile number'});if(r.reset<Date.now())r={count:0,reset:Date.now()+10*60e3};if(r.count>=3)return json(res,429,{error:'Too many OTP requests. Try again in a few minutes.'});r.count++;otpRequests.set(key,r);const code=String(crypto.randomInt(100000,1000000));otps.set(phone,{code,expires:Date.now()+5*60e3,attempts:0});console.log(`[DEMO OTP] ${phone}: ${code}`);return json(res,200,{ok:true,message:'OTP sent. It expires in 5 minutes.',...(process.env.NODE_ENV!=='production'?{demoOtp:code}: {})})}
  if(req.method==='POST'&&p==='/api/otp/verify'){const b=await body(req),phone=String(b.phone||''),o=otps.get(phone);if(!o||o.expires<Date.now()){otps.delete(phone);return json(res,401,{error:'Invalid or expired OTP'})}if(o.code!==String(b.code||'')){o.attempts++;if(o.attempts>=5)otps.delete(phone);return json(res,401,{error:'Invalid or expired OTP'})}otps.delete(phone);const sid=crypto.randomBytes(32).toString('hex');farmerSessions.set(sid,{phone,expires:Date.now()+15*60e3});res.setHeader('Set-Cookie',`cs_farmer_session=${sid}; HttpOnly; SameSite=Strict; Path=/; Max-Age=900`);return json(res,200,{verified:true})}
